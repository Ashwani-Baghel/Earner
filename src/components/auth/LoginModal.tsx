...
export function LoginModal({ open, onClose, onSwitchToRegister }: LoginModalProps) {
  const { signIn, signInWithGoogle, resetPassword, error, clearError, loading, isFirebaseReady } = useAuth();
  ...
  const redirectAfterLogin = async () => {
    try {
      ...
      throw err; // throw back to handleSubmit so it can toast the error
    } 
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {!isForgotPassword && (
            <div>
              ...
              <div className="flex justify-end mt-1.5">
                <button type="button" onClick={() => setIsForgotPassword(true)} className="text-xs text-[#1dbf73] font-medium hover:underline">
                  Forgot Password?
                </button>
              </div>
            </div>
          ...
          {isForgotPassword && (
            <div className="text-center mt-2">
              <button type="button" onClick={() => setIsForgotPassword(false)} className="text-sm font-medium text-[#74767e] hover:text-[#404145] transition-colors">
                Back to Sign In
              </button>
            </div>
          )}
